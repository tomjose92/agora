...
describe("message drafts", () => {
  ...
  it("isolates conversation text and removes empty entries", () => {
    useMessageDrafts.getState().setDraft("channel-a", "draft A ");
    useMessageDrafts.getState().setDraft("channel-b", "draft B");
    ...
    useMessageDrafts.getState().setDraft("channel-a", "");
  ...
  it("keeps clearing an absent draft as a no-op", () => {
    useMessageDrafts.getState().setDraft("missing", "");
    expect(useMessageDrafts.getState().byConvo).toEqual({});
  });
});
