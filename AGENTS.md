...
| `ui/app2/` | **Generated** build of `web/` (never hand-edit — run `npm run build`). Served at `/app2/` by the same static serving; the vanilla UI stays the default at `/`. |
...
| `web/` | React (Vite + TS) port of the web UI, at core-chat parity. Builds into `ui/app2/`; reuses `ui/style.css` and the same class names. Voice/live/TTS not ported yet. |
...
The vanilla web UI needs no build — edit `ui/*.js` and reload. The React UI
at `/app2/` does: `ui/app2/` is generated by `npm run build -w web` — never
hand-edit it, and rebuild it in any PR that touches `web/` or
`packages/core`. `scripts/gen-emoji.js` writes three outputs (`ui/emoji.js`,
