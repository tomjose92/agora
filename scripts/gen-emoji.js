/* Generates ui/emoji.js and mobile/src/lib/emoji.ts from the single curated
   dataset below, so both clients ship identical emoji data. Edit DATA here,
   then run `node scripts/gen-emoji.js`; mobile/tests/emoji.test.ts asserts
   the two outputs stayed in sync. */
const fs = require("fs");
const path = require("path");
const ROOT = path.join(__dirname, "..");

const DATA = [
  ["Smileys", [
    ["😀", "grinning face smile happy"],
    ["😃", "grinning big eyes smile happy"],
    ["😄", "grinning smiling eyes happy laugh"],
    ["😁", "beaming grin smile"],
    ["😆", "laughing squinting haha lol"],
    ["😅", "sweat smile nervous phew"],
    ["🤣", "rofl rolling laughing lol"],
    ["😂", "joy tears laughing lol funny"],
    ["🙂", "slight smile ok"],
    ["🙃", "upside down silly sarcasm"],
    ["😉", "wink flirt"],
    ["😊", "smiling blush happy warm"],
    ["😇", "halo angel innocent"],
    ["🥰", "hearts love adore smiling"],
    ["😍", "heart eyes love crush"],
    ["🤩", "star struck excited wow"],
    ["😘", "blowing kiss love"],
    ["😗", "kissing"],
    ["😚", "kissing closed eyes"],
    ["🥲", "smiling tear grateful bittersweet"],
    ["😋", "yum savoring delicious tasty"],
    ["😛", "tongue out playful"],
    ["😜", "winking tongue zany playful"],
    ["🤪", "zany crazy goofy wild"],
    ["😝", "squinting tongue playful"],
    ["🤑", "money mouth rich dollar"],
    ["🤗", "hug hugging warm"],
    ["🤭", "hand over mouth giggle oops"],
    ["🤫", "shushing quiet secret"],
    ["🤔", "thinking hmm consider"],
    ["🤐", "zipper mouth secret quiet"],
    ["🤨", "raised eyebrow skeptical suspicious"],
    ["😐", "neutral meh"],
    ["😑", "expressionless blank"],
    ["😶", "no mouth speechless silence"],
    ["😏", "smirk sly suggestive"],
    ["😒", "unamused annoyed meh"],
    ["🙄", "eye roll whatever"],
    ["😬", "grimace awkward eek"],
    ["🤥", "lying pinocchio liar"],
    ["😌", "relieved calm content"],
    ["😔", "pensive sad thoughtful"],
    ["😪", "sleepy tired"],
    ["🤤", "drooling want hungry"],
    ["😴", "sleeping zzz asleep"],
    ["😷", "mask sick medical"],
    ["🤒", "sick thermometer fever ill"],
    ["🤕", "head bandage hurt injured"],
    ["🤢", "nauseated sick green gross"],
    ["🤮", "vomiting sick puke gross"],
    ["🤧", "sneezing sick tissue"],
    ["🥵", "hot sweating heat"],
    ["🥶", "cold freezing frozen"],
    ["🥴", "woozy drunk dizzy tipsy"],
    ["😵", "dizzy knocked out"],
    ["🤯", "exploding head mind blown shocked"],
    ["🤠", "cowboy hat yeehaw"],
    ["🥳", "party celebrate birthday"],
    ["🥸", "disguise glasses incognito"],
    ["😎", "sunglasses cool"],
    ["🤓", "nerd glasses geek"],
    ["🧐", "monocle inspect fancy"],
    ["😕", "confused unsure"],
    ["😟", "worried concerned"],
    ["🙁", "slightly frowning sad"],
    ["😮", "open mouth surprised wow"],
    ["😯", "hushed surprised"],
    ["😲", "astonished shocked amazed"],
    ["😳", "flushed embarrassed blush"],
    ["🥺", "pleading puppy eyes begging cute"],
    ["😦", "frowning open mouth"],
    ["😨", "fearful scared afraid"],
    ["😰", "anxious sweat nervous"],
    ["😥", "sad relieved disappointed"],
    ["😢", "crying sad tear"],
    ["😭", "loudly crying sobbing sad"],
    ["😱", "screaming fear shocked horror"],
    ["😖", "confounded frustrated"],
    ["😣", "persevering struggling"],
    ["😞", "disappointed sad"],
    ["😓", "downcast sweat tired"],
    ["😩", "weary tired ugh"],
    ["😫", "tired exhausted"],
    ["🥱", "yawning bored tired sleepy"],
    ["😤", "triumph steam huffing frustrated"],
    ["😡", "pouting angry rage mad"],
    ["😠", "angry mad"],
    ["🤬", "cursing swearing angry symbols"],
    ["😈", "devil smiling imp evil"],
    ["👿", "angry imp devil"],
    ["💀", "skull dead death"],
    ["☠️", "skull crossbones danger dead"],
    ["💩", "poop pile crap"],
    ["🤡", "clown joker"],
    ["👹", "ogre monster japanese"],
    ["👻", "ghost boo halloween spooky"],
    ["👽", "alien ufo extraterrestrial"],
    ["🤖", "robot bot"],
    ["🙈", "see no evil monkey hide"],
    ["🙉", "hear no evil monkey"],
    ["🙊", "speak no evil monkey oops"],
    ["💋", "kiss mark lips"],
    ["💤", "zzz sleeping snore"],
  ]],
  ["People", [
    ["👋", "wave hello goodbye hi bye"],
    ["🤚", "raised back of hand stop"],
    ["✋", "raised hand high five stop"],
    ["🖖", "vulcan salute spock"],
    ["👌", "ok hand perfect"],
    ["🤌", "pinched fingers chef kiss italian"],
    ["🤏", "pinching small tiny bit"],
    ["✌️", "victory peace"],
    ["🤞", "crossed fingers luck hope"],
    ["🤟", "love you gesture"],
    ["🤘", "horns rock metal"],
    ["🤙", "call me shaka hang loose"],
    ["👈", "point left"],
    ["👉", "point right"],
    ["👆", "point up"],
    ["👇", "point down"],
    ["☝️", "index up one"],
    ["👍", "thumbs up yes like approve good"],
    ["👎", "thumbs down no dislike bad"],
    ["✊", "raised fist power solidarity"],
    ["👊", "fist bump punch"],
    ["🤛", "left fist bump"],
    ["🤜", "right fist bump"],
    ["👏", "clap applause bravo"],
    ["🙌", "raising hands hooray celebration praise"],
    ["👐", "open hands"],
    ["🤲", "palms up together please"],
    ["🤝", "handshake deal agreement"],
    ["🙏", "folded hands thanks please pray namaste"],
    ["✍️", "writing hand"],
    ["💅", "nail polish sassy"],
    ["🤳", "selfie"],
    ["💪", "flex biceps strong muscle"],
    ["👀", "eyes look watching"],
    ["👁️", "eye"],
    ["🧠", "brain smart mind"],
    ["🦷", "tooth dentist"],
    ["👅", "tongue"],
    ["👄", "mouth lips"],
    ["👶", "baby infant"],
    ["🧒", "child kid"],
    ["🧑", "person adult"],
    ["👨", "man male"],
    ["👩", "woman female"],
    ["🧓", "older person elderly"],
    ["🙋", "raising hand volunteer question"],
    ["🤷", "shrug dunno whatever idk"],
    ["🤦", "facepalm smh doh"],
    ["🙇", "bowing sorry thanks"],
    ["💁", "tipping hand sassy information"],
    ["🙅", "no gesture nope forbidden"],
    ["🙆", "ok gesture yes"],
    ["🧑‍💻", "technologist coder programmer developer"],
    ["🚶", "walking pedestrian"],
    ["🏃", "running runner rush"],
    ["💃", "dancing woman salsa"],
    ["🕺", "dancing man disco"],
    ["👯", "bunny ears dancers party twins"],
    ["🧘", "meditation lotus yoga zen"],
    ["🛀", "bath relaxing"],
  ]],
  ["Nature", [
    ["🐶", "dog puppy pet"],
    ["🐱", "cat kitten pet"],
    ["🐭", "mouse"],
    ["🐹", "hamster"],
    ["🐰", "rabbit bunny"],
    ["🦊", "fox"],
    ["🐻", "bear"],
    ["🐼", "panda"],
    ["🐨", "koala"],
    ["🐯", "tiger"],
    ["🦁", "lion"],
    ["🐮", "cow"],
    ["🐷", "pig"],
    ["🐸", "frog"],
    ["🐵", "monkey"],
    ["🐔", "chicken"],
    ["🐧", "penguin"],
    ["🐦", "bird"],
    ["🐤", "baby chick"],
    ["🦆", "duck"],
    ["🦅", "eagle"],
    ["🦉", "owl"],
    ["🦇", "bat"],
    ["🐺", "wolf"],
    ["🐴", "horse"],
    ["🦄", "unicorn magic"],
    ["🐝", "bee honeybee"],
    ["🐛", "bug caterpillar"],
    ["🦋", "butterfly"],
    ["🐌", "snail slow"],
    ["🐞", "lady beetle ladybug"],
    ["🐜", "ant"],
    ["🐢", "turtle slow"],
    ["🐍", "snake"],
    ["🦎", "lizard gecko"],
    ["🦖", "t-rex dinosaur"],
    ["🐙", "octopus"],
    ["🦀", "crab"],
    ["🐬", "dolphin"],
    ["🐳", "whale"],
    ["🦈", "shark"],
    ["🐊", "crocodile alligator"],
    ["🐘", "elephant"],
    ["🦒", "giraffe"],
    ["🌵", "cactus desert"],
    ["🌲", "evergreen tree pine"],
    ["🌴", "palm tree tropical"],
    ["🌱", "seedling sprout growth"],
    ["🍀", "four leaf clover luck"],
    ["🌿", "herb leaf"],
    ["🍁", "maple leaf fall autumn"],
    ["🍂", "fallen leaves autumn"],
    ["💐", "bouquet flowers"],
    ["🌷", "tulip flower"],
    ["🌹", "rose flower"],
    ["🌸", "cherry blossom flower spring"],
    ["🌼", "blossom flower daisy"],
    ["🌻", "sunflower"],
    ["🌞", "sun with face"],
    ["🌝", "full moon face"],
    ["🌙", "crescent moon night"],
    ["⭐", "star"],
    ["🌟", "glowing star shining"],
    ["✨", "sparkles shiny magic new"],
    ["⚡", "lightning zap fast electric"],
    ["🔥", "fire lit hot flame"],
    ["🌈", "rainbow pride"],
    ["☀️", "sun sunny"],
    ["⛅", "partly cloudy sun cloud"],
    ["☁️", "cloud cloudy"],
    ["🌧️", "rain cloud"],
    ["❄️", "snowflake cold winter snow"],
    ["⛄", "snowman winter"],
    ["🌊", "wave ocean water sea"],
    ["💧", "droplet water"],
    ["🌍", "earth globe world europe africa"],
    ["🌎", "earth globe world americas"],
  ]],
  ["Food", [
    ["🍏", "green apple fruit"],
    ["🍎", "red apple fruit"],
    ["🍊", "tangerine orange fruit"],
    ["🍋", "lemon sour fruit"],
    ["🍌", "banana fruit"],
    ["🍉", "watermelon fruit"],
    ["🍇", "grapes fruit"],
    ["🍓", "strawberry fruit"],
    ["🫐", "blueberries fruit"],
    ["🍒", "cherries fruit"],
    ["🍑", "peach fruit butt"],
    ["🥭", "mango fruit"],
    ["🍍", "pineapple fruit"],
    ["🥥", "coconut"],
    ["🥝", "kiwi fruit"],
    ["🍅", "tomato"],
    ["🥑", "avocado"],
    ["🥦", "broccoli vegetable"],
    ["🥕", "carrot vegetable"],
    ["🌽", "corn maize"],
    ["🌶️", "hot pepper spicy chili"],
    ["🥔", "potato"],
    ["🍞", "bread loaf"],
    ["🥐", "croissant"],
    ["🥨", "pretzel"],
    ["🧀", "cheese"],
    ["🥚", "egg"],
    ["🍳", "fried egg cooking breakfast"],
    ["🥞", "pancakes breakfast"],
    ["🧇", "waffle breakfast"],
    ["🥓", "bacon"],
    ["🍗", "poultry leg chicken drumstick"],
    ["🌭", "hot dog"],
    ["🍔", "hamburger burger"],
    ["🍟", "fries french fries"],
    ["🍕", "pizza slice"],
    ["🥪", "sandwich"],
    ["🌮", "taco mexican"],
    ["🌯", "burrito wrap"],
    ["🥗", "salad green healthy"],
    ["🍝", "spaghetti pasta"],
    ["🍜", "ramen noodles soup"],
    ["🍣", "sushi japanese"],
    ["🍤", "fried shrimp tempura"],
    ["🍙", "rice ball onigiri"],
    ["🍚", "rice"],
    ["🍛", "curry"],
    ["🥟", "dumpling gyoza"],
    ["🍦", "soft ice cream"],
    ["🍨", "ice cream sundae"],
    ["🍰", "cake slice shortcake dessert"],
    ["🎂", "birthday cake celebration"],
    ["🧁", "cupcake"],
    ["🍫", "chocolate bar"],
    ["🍬", "candy sweet"],
    ["🍭", "lollipop"],
    ["🍩", "doughnut donut"],
    ["🍪", "cookie"],
    ["🍿", "popcorn movie"],
    ["☕", "coffee hot beverage"],
    ["🍵", "tea green matcha"],
    ["🥤", "cup with straw soda"],
    ["🧃", "juice box"],
    ["🍺", "beer mug"],
    ["🍻", "beers cheers clinking"],
    ["🥂", "clinking glasses champagne toast cheers"],
    ["🍷", "wine glass"],
    ["🥃", "whisky tumbler scotch"],
    ["🍸", "cocktail martini"],
    ["🍹", "tropical drink"],
    ["🍾", "champagne bottle celebration pop"],
  ]],
  ["Activities", [
    ["⚽", "soccer football"],
    ["🏀", "basketball"],
    ["🏈", "american football"],
    ["⚾", "baseball"],
    ["🎾", "tennis"],
    ["🏐", "volleyball"],
    ["🏉", "rugby"],
    ["🎱", "8 ball billiards pool"],
    ["🏓", "ping pong table tennis"],
    ["🏸", "badminton"],
    ["🏒", "ice hockey"],
    ["⛳", "golf flag hole"],
    ["🏹", "bow arrow archery"],
    ["🎣", "fishing pole"],
    ["🥊", "boxing glove fight"],
    ["🥋", "martial arts karate judo"],
    ["⛸️", "ice skate"],
    ["🎿", "skis skiing"],
    ["🛹", "skateboard"],
    ["🏄", "surfing surfer"],
    ["🏊", "swimming swimmer"],
    ["🚴", "biking cyclist"],
    ["🏋️", "weight lifting gym"],
    ["🤸", "cartwheel gymnastics"],
    ["🧗", "climbing"],
    ["🏆", "trophy winner champion"],
    ["🥇", "gold medal first"],
    ["🥈", "silver medal second"],
    ["🥉", "bronze medal third"],
    ["🏅", "sports medal"],
    ["🎫", "ticket"],
    ["🎭", "performing arts theater masks"],
    ["🎨", "art palette paint"],
    ["🎬", "clapper board movie film"],
    ["🎤", "microphone sing karaoke"],
    ["🎧", "headphones music"],
    ["🎹", "piano keyboard music"],
    ["🥁", "drum music"],
    ["🎷", "saxophone music"],
    ["🎺", "trumpet music"],
    ["🎸", "guitar music rock"],
    ["🎻", "violin music"],
    ["🎲", "die dice game random"],
    ["♟️", "chess pawn strategy"],
    ["🎯", "dart bullseye target"],
    ["🎳", "bowling"],
    ["🎮", "video game controller gaming"],
    ["🎰", "slot machine casino"],
    ["🧩", "puzzle piece jigsaw"],
    ["🎉", "party popper celebration congrats tada"],
    ["🎊", "confetti ball celebration"],
    ["🎈", "balloon party"],
    ["🎁", "gift present wrapped"],
    ["🎀", "ribbon bow"],
  ]],
  ["Travel", [
    ["🚗", "car automobile"],
    ["🚕", "taxi cab"],
    ["🚙", "suv recreational vehicle"],
    ["🚌", "bus"],
    ["🏎️", "race car fast"],
    ["🚓", "police car"],
    ["🚑", "ambulance"],
    ["🚒", "fire engine truck"],
    ["🚚", "delivery truck"],
    ["🚜", "tractor farm"],
    ["🛴", "kick scooter"],
    ["🚲", "bicycle bike"],
    ["🛵", "motor scooter vespa"],
    ["🏍️", "motorcycle"],
    ["🚨", "police light siren alert emergency"],
    ["🚂", "locomotive train steam"],
    ["🚆", "train"],
    ["🚇", "metro subway"],
    ["✈️", "airplane flight travel"],
    ["🛫", "airplane departure takeoff"],
    ["🚀", "rocket launch ship space fast"],
    ["🛸", "flying saucer ufo"],
    ["🚁", "helicopter"],
    ["⛵", "sailboat sailing"],
    ["🚤", "speedboat"],
    ["🚢", "ship cruise"],
    ["⚓", "anchor"],
    ["🗺️", "world map"],
    ["🗽", "statue of liberty new york"],
    ["🏰", "castle"],
    ["🎡", "ferris wheel"],
    ["🎢", "roller coaster"],
    ["⛺", "tent camping"],
    ["🏠", "house home"],
    ["🏡", "house with garden home"],
    ["🏢", "office building work"],
    ["🏥", "hospital"],
    ["🏦", "bank"],
    ["🏫", "school"],
    ["⛪", "church"],
    ["🕌", "mosque"],
    ["🗿", "moai statue stone face"],
    ["🌋", "volcano eruption"],
    ["⛰️", "mountain"],
    ["🏔️", "snow capped mountain"],
    ["🏖️", "beach umbrella vacation"],
    ["🏝️", "desert island tropical"],
    ["🌅", "sunrise"],
    ["🌇", "sunset city"],
    ["🌃", "night city stars"],
    ["🌉", "bridge at night"],
    ["🌌", "milky way galaxy space"],
  ]],
  ["Objects", [
    ["⌚", "watch time"],
    ["📱", "mobile phone smartphone"],
    ["💻", "laptop computer"],
    ["⌨️", "keyboard typing"],
    ["🖥️", "desktop computer monitor"],
    ["🖨️", "printer"],
    ["🖱️", "computer mouse"],
    ["💾", "floppy disk save"],
    ["💿", "optical disk cd"],
    ["📷", "camera photo"],
    ["📸", "camera flash photo"],
    ["📹", "video camera"],
    ["🎥", "movie camera film"],
    ["📞", "telephone receiver call"],
    ["☎️", "telephone phone"],
    ["📺", "television tv"],
    ["📻", "radio"],
    ["🎙️", "studio microphone podcast"],
    ["⏰", "alarm clock time wake"],
    ["⏳", "hourglass time waiting"],
    ["📡", "satellite antenna signal"],
    ["🔋", "battery power"],
    ["🔌", "electric plug power"],
    ["💡", "light bulb idea"],
    ["🔦", "flashlight torch"],
    ["🕯️", "candle"],
    ["🗑️", "wastebasket trash delete"],
    ["💵", "dollar banknote money cash"],
    ["💰", "money bag rich"],
    ["💳", "credit card payment"],
    ["💎", "gem diamond jewel"],
    ["⚖️", "balance scale justice law"],
    ["🔧", "wrench tool fix"],
    ["🔨", "hammer tool build"],
    ["🛠️", "hammer wrench tools repair"],
    ["⚙️", "gear settings cog"],
    ["🧲", "magnet attract"],
    ["💣", "bomb explosive"],
    ["🛡️", "shield protection security"],
    ["🔮", "crystal ball fortune magic"],
    ["🔭", "telescope astronomy"],
    ["🔬", "microscope science lab"],
    ["💊", "pill medicine drug"],
    ["💉", "syringe injection vaccine"],
    ["🩹", "adhesive bandage band-aid"],
    ["🌡️", "thermometer temperature fever"],
    ["🧬", "dna genetics biology"],
    ["🦠", "microbe virus germ bacteria"],
    ["🧪", "test tube science experiment"],
    ["🧹", "broom sweep clean"],
    ["🚿", "shower"],
    ["🛁", "bathtub bath"],
    ["🔑", "key unlock access"],
    ["🚪", "door"],
    ["🪑", "chair seat"],
    ["🛏️", "bed sleep"],
    ["🛋️", "couch sofa lounge"],
    ["🧸", "teddy bear plush toy"],
    ["🖼️", "framed picture painting"],
    ["🛍️", "shopping bags"],
    ["🛒", "shopping cart"],
    ["✉️", "envelope email letter mail"],
    ["📦", "package box delivery"],
    ["📫", "mailbox mail"],
    ["📝", "memo note pencil writing"],
    ["✏️", "pencil"],
    ["🖊️", "pen"],
    ["🖌️", "paintbrush"],
    ["📁", "file folder"],
    ["📂", "open file folder"],
    ["📅", "calendar date"],
    ["📈", "chart increasing up growth stonks"],
    ["📉", "chart decreasing down loss"],
    ["📊", "bar chart statistics data"],
    ["📋", "clipboard"],
    ["📌", "pushpin pin location"],
    ["📎", "paperclip attachment"],
    ["📏", "straight ruler measure"],
    ["✂️", "scissors cut"],
    ["🔒", "locked padlock secure private"],
    ["🔓", "unlocked open"],
    ["📖", "open book reading"],
    ["📚", "books library study"],
    ["📓", "notebook"],
    ["🔖", "bookmark"],
    ["📰", "newspaper news"],
  ]],
  ["Symbols", [
    ["❤️", "red heart love"],
    ["🧡", "orange heart"],
    ["💛", "yellow heart"],
    ["💚", "green heart"],
    ["💙", "blue heart"],
    ["💜", "purple heart"],
    ["🖤", "black heart"],
    ["🤍", "white heart"],
    ["🤎", "brown heart"],
    ["💔", "broken heart heartbreak sad"],
    ["❣️", "heart exclamation"],
    ["💕", "two hearts love"],
    ["💞", "revolving hearts love"],
    ["💓", "beating heart love"],
    ["💗", "growing heart love"],
    ["💖", "sparkling heart love"],
    ["💘", "heart with arrow cupid"],
    ["💝", "heart with ribbon gift"],
    ["💯", "hundred points perfect score keep it real"],
    ["💢", "anger symbol mad"],
    ["💥", "collision boom explosion bang"],
    ["💫", "dizzy stars"],
    ["💦", "sweat droplets splash water"],
    ["💨", "dashing away fast wind gone"],
    ["☮️", "peace symbol"],
    ["♻️", "recycling recycle green"],
    ["✅", "check mark button done yes correct"],
    ["☑️", "check box ballot"],
    ["✔️", "check mark yes done"],
    ["❌", "cross mark no wrong x"],
    ["❎", "cross mark button no"],
    ["➕", "plus add"],
    ["➖", "minus subtract"],
    ["➗", "divide division"],
    ["✖️", "multiply times x"],
    ["❓", "question mark"],
    ["❗", "exclamation mark important"],
    ["‼️", "double exclamation important"],
    ["⁉️", "exclamation question interrobang"],
    ["⚠️", "warning caution alert"],
    ["🚫", "prohibited forbidden no banned"],
    ["🆗", "ok button okay"],
    ["🆒", "cool button"],
    ["🆕", "new button"],
    ["🔴", "red circle"],
    ["🟠", "orange circle"],
    ["🟡", "yellow circle"],
    ["🟢", "green circle"],
    ["🔵", "blue circle"],
    ["🟣", "purple circle"],
    ["⚫", "black circle"],
    ["⚪", "white circle"],
    ["🔔", "bell notification"],
    ["🔕", "bell slash mute silent"],
    ["🎵", "musical note music"],
    ["🎶", "musical notes music melody"],
    ["🔄", "arrows refresh repeat sync"],
    ["🏁", "checkered flag finish race done"],
    ["🚩", "triangular flag red flag warning"],
    ["🏳️‍🌈", "rainbow flag pride lgbtq"],
  ]],
];

function fmtCategories(indent) {
  return DATA.map(([name, list]) => {
    const rows = list.map(([e, k]) => `${indent}    [${JSON.stringify(e)}, ${JSON.stringify(k)}],`).join("\n");
    return `${indent}  {\n${indent}    name: ${JSON.stringify(name)},\n${indent}    emoji: [\n${rows}\n${indent}    ],\n${indent}  },`;
  }).join("\n");
}

const webHeader = `/* Curated Unicode emoji set for the composer picker, in keyboard order.
   Each entry is [emoji, search keywords]. Generated by scripts/gen-emoji.js
   together with mobile/src/lib/emoji.ts — edit there, not here. */
const AGO_EMOJI = [
`;
fs.writeFileSync(path.join(ROOT, "ui/emoji.js"),
  webHeader.replace("[\n", "[\n") + fmtCategories("") + "\n];\n");

const tsHeader = `/* Curated Unicode emoji set for the composer picker, in keyboard order.
   Each entry is [emoji, search keywords]. Generated by scripts/gen-emoji.js
   together with ui/emoji.js — edit there, not here. */

export type EmojiEntry = [emoji: string, keywords: string];

export interface EmojiCategory {
  name: string;
  emoji: EmojiEntry[];
}

export const EMOJI_CATEGORIES: EmojiCategory[] = [
`;
fs.writeFileSync(path.join(ROOT, "mobile/src/lib/emoji.ts"),
  tsHeader + fmtCategories("") + "\n];\n");

const total = DATA.reduce((n, [, l]) => n + l.length, 0);
console.log(`wrote ${DATA.length} categories, ${total} emoji`);
