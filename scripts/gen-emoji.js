/* Generates ui/emoji.js and packages/core/src/lib/emoji.ts from the single
   curated dataset below, so every client ships identical emoji data (mobile
   and the React web UI both import @agora/core's copy). Edit DATA here,
   then run `node scripts/gen-emoji.js`; the emoji sync tests in
   packages/core/tests and mobile/tests assert the outputs stayed aligned. */
const fs = require("fs");
...
function fmtCategories(indent) {
  return DATA.map(([name, list]) => {
    const rows = list.map(([e, k]) => `${indent}    [${JSON.stringify(e)}, ${JSON.stringify(k)}],`).join("\n");
    return `${indent}  {\n${indent}    name: ${JSON.stringify(name)},\n${indent}    emoji: [\n${rows}\n${indent}    ],\n${indent}  },`;
  }).join("\n");
}

const webHeader = `/* Curated Unicode emoji set for the composer picker, in keyboard order.
   Each entry is [emoji, search keywords]. Generated by scripts/gen-emoji.js
   together with packages/core/src/lib/emoji.ts — edit there, not here. */
const AGO_EMOJI = [
`;
fs.writeFileSync(path.join(ROOT, "ui/emoji.js"),
  webHeader.replace("[\n", "[\n") + fmtCategories("") + "\n];\n");

const coreHeader = `/* Curated Unicode emoji set for the composer picker, in keyboard order.
   Each entry is [emoji, search keywords]. Generated by scripts/gen-emoji.js
   together with ui/emoji.js — edit there, not here. */
...
`;
fs.writeFileSync(path.join(ROOT, "packages/core/src/lib/emoji.ts"),
  coreHeader + fmtCategories("") + "\n];\n");

const total = DATA.reduce((n, [, l]) => n + l.length, 0);
console.log(`wrote ${DATA.length} categories, ${total} emoji`);
